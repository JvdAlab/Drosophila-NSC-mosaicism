...
public class CellCntrImageCanvas extends ImageCanvas {
	...
	//modified to draw bigger markers - RB
	@Override
		...
		while (it.hasNext()) {
			...
			while (mit.hasNext()) {
				...
				if (sameSlice || showAll) {
					...
					g2.drawOval((int) xM - markerR, (int) yM - markerR, 2*markerR, 2*markerR);
					if (showNumbers) g2.drawString(Integer.toString(typeID),
						(int) xM + markerR, (int) yM - markerR);
				}
			}
		}
	}
		...
		while (it.hasNext()) {
			...
			while (mit.hasNext()) {
				...
				if (m.getZ() == img.getCurrentSlice()) {
					xM = m.getX();
					yM = m.getY();
					g2r.drawOval((int) xM - markerR, (int) yM - markerR, 2*markerR, 2*markerR);
