...
public class CellCounter extends JFrame implements ActionListener, ItemListener
{
	...
	private CellCounterOptions options;
	...
	private void initGUI() {
		...
		//don't add buttons for adding or removing types, keep to default 8 types - RB
		/*gbc = new GridBagConstraints();
		gbc.anchor = GridBagConstraints.NORTHWEST;
		gbc.fill = GridBagConstraints.BOTH;
		gbc.gridx = 0;
	...
	public void populateTxtFields() {
		...
		while (it.hasNext()) {
			...
			if (txtFieldVector.size() > index) {
				...
				if(options==null) options = new CellCounterOptions(ic);
				Color colour = options.getColour(index+1);
				tArea.setBackground(colour);
				dynRadioVector.get(index).setBackground(colour);
			}
		}
		validateLayout();
		//repaint();
	...
	public ImagePlus getCounterImage(){
		return counterImg;
	}
